...
def processing_loop(csvfile):
    ...
    while NUMBER_ACCOUNT <= END_COUNT:
#      print(START_COUNT)
#      START_COUNT += 1
       print()
...
#      account_field = driver.find_element(By.CSS_SELECTOR, "form-control")
#      account_field.send_keys(username)
       account_field.send_keys(NEW_ADDRESS)
       sleep(5)
       login_button = driver.find_element(By.ID, "submit-btn")
#      login_button = driver.find_element(By.CSS_SELECTOR, "g-recaptcha btn btn-success border-rad")
       login_button.click()
...
#     archwayd tx bank send archway1fg93ekk3mzrq7qcuq2grfps9j72ju39kfcqa02 archway1mdlzc9wg5806guru66xm8x3kvtfqhawkmu47q7 1utorii --fees 0utorii --chain-id torii-1 -y --output json
       cmd = f"archwayd tx bank send "+NEW_ADDRESS+" "+MAIN_ACCOUNT+" 3000000utorii --fees 0utorii --chain-id torii-1 -y --output json"
       print(cmd)
       child = spawn(cmd, timeout=5, encoding='utf-8')
       child.expect('Enter keyring passphrase:')
       child.sendline(PASSWORD)
...
#      csv_writer.writerow([light, voltage])
       csv_writer.writerow(['archwayd keys add', NEW_NAME, '', 'archwayd tx bank send', NEW_ADDRESS, MAIN_ACCOUNT, '', 0, '', NEW_MNEMONIC])
       csvfile.flush()
       time.sleep(1)
